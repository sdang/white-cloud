module DcSummariesHelper
  def import_buttons(prefix, dc_summary_id)
    ...
    html += link_to "import from last d/c summary", url_for(:controller => "/dc_summaries", :action => "import_handler", :prefix => prefix, :from => "dc", :id => dc_summary_id), :class => "button white small", :remote => true, :id => "#{prefix}-import-dc-summary"
